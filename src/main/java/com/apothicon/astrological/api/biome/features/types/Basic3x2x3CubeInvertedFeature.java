...
public class Basic3x2x3CubeInvertedFeature extends Feature<SimpleBlockConfiguration> {
    public Basic3x2x3CubeInvertedFeature(Codec<SimpleBlockConfiguration> pContext) {
        super(pContext);
    }
    ...
    public boolean place(FeaturePlaceContext<SimpleBlockConfiguration> pContext) {
        ...
        if (worldGenLevel.isEmptyBlock(blockPos.offset(-1, 1, 0)) || worldGenLevel.isEmptyBlock(blockPos.offset(1, 1, 0)) ||
            ...
            generateCube(worldGenLevel, blockPos.below(2), material);
            generateCube(worldGenLevel, blockPos.below(4), material);
            if ((int)(random.nextFloat()*(3)+1) < 2) {
                generateCube(worldGenLevel, blockPos.below(6), material);
                if ((int)(random.nextFloat()*(4)+1) < 2) {
                    generateCube(worldGenLevel, blockPos.below(8), material);
                    if ((int)(random.nextFloat()*(4)+1) < 2) {
                        generateCube(worldGenLevel, blockPos.below(10), material);
                        if ((int)(random.nextFloat()*(4)+1) < 2) {
                            generateCube(worldGenLevel, blockPos.below(12), material);
                            if ((int)(random.nextFloat()*(4)+1) < 2) {
                                generateCube(worldGenLevel, blockPos.below(14), material);
                                if ((int)(random.nextFloat()*(4)+1) < 2) {
                                    generateCube(worldGenLevel, blockPos.below(16), material);
                                }
                            }
                        }
                    }
    ...
    private static void generateCube(WorldGenLevel worldGenLevel, BlockPos blockPos, BlockState material) {
        ...
        generateLayer(worldGenLevel, blockPos.below(), material);
    }
