...
public class LargePackedIcePillarFeature extends Feature<LargeDripstoneConfiguration> {
    public LargePackedIcePillarFeature(Codec<LargeDripstoneConfiguration> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargePackedIcePillarFeature.LargePillar largepillarfeature$largepillar = makeIce(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePackedIcePillarFeature.LargePillar largepillarfeature$largepillar1 = makeIce(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
                    LargePackedIcePillarFeature.WindOffsetter largepillarfeature$windoffsetter;
                    if (largepillarfeature$largepillar.isSuitableForWind(config) && largepillarfeature$largepillar1.isSuitableForWind(config)) {
                        largepillarfeature$windoffsetter = new LargePackedIcePillarFeature.WindOffsetter(blockpos.getY(), random, config.windSpeed);
                    } else {
                        largepillarfeature$windoffsetter = LargePackedIcePillarFeature.WindOffsetter.noWind();
                    }
                    ...
                    if (flag) {
                        largepillarfeature$largepillar.placeBlocks(worldgenlevel, random, largepillarfeature$windoffsetter, k);
                    }

                    if (flag1) {
                        largepillarfeature$largepillar1.placeBlocks(worldgenlevel, random, largepillarfeature$windoffsetter, k);
                    }
    ...
    private static LargePackedIcePillarFeature.LargePillar makeIce(BlockPos pRoot, boolean pPointingUp, RandomSource pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
        return new LargePackedIcePillarFeature.LargePillar(pRoot, pPointingUp, pRadius, (double)pBluntnessBase.sample(pRandom), (double)pScaleBase.sample(pRandom));
    }
    ...
    static final class LargePillar {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel pLevel, LargePackedIcePillarFeature.WindOffsetter pWindOffsetter) {
            while(this.radius > 1) {
        ...
        void placeBlocks(WorldGenLevel pLevel, RandomSource pRandom, LargePackedIcePillarFeature.WindOffsetter pWindOffsetter, int pRadius) {
    ...
    static final class WindOffsetter {
        ...
        static LargePackedIcePillarFeature.WindOffsetter noWind() {
            return new LargePackedIcePillarFeature.WindOffsetter();
        }
