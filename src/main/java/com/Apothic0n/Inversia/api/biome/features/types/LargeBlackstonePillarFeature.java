...
public class LargeBlackstonePillarFeature extends Feature<LargeDripstoneConfiguration> {
    public LargeBlackstonePillarFeature(Codec<LargeDripstoneConfiguration> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargeBlackstonePillarFeature.LargePillar largepillarfeature$largepillar = makeBlackstone(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargeBlackstonePillarFeature.LargePillar largepillarfeature$largepillar1 = makeBlackstone(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
                    LargeBlackstonePillarFeature.WindOffsetter largepillarfeature$windoffsetter;
                    if (largepillarfeature$largepillar.isSuitableForWind(config) && largepillarfeature$largepillar1.isSuitableForWind(config)) {
                        largepillarfeature$windoffsetter = new LargeBlackstonePillarFeature.WindOffsetter(blockpos.getY(), random, config.windSpeed);
                    } else {
                        largepillarfeature$windoffsetter = LargeBlackstonePillarFeature.WindOffsetter.noWind();
                    }
    ...
    static final class LargePillar {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel pLevel, LargeBlackstonePillarFeature.WindOffsetter pWindOffsetter) {
            while(this.radius > 1) {
    ...
    static final class WindOffsetter {
        ...
        static LargeBlackstonePillarFeature.WindOffsetter noWind() {
            return new LargeBlackstonePillarFeature.WindOffsetter();
        }
