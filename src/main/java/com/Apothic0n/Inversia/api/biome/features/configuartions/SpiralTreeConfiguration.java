...
public class SpiralTreeConfiguration implements FeatureConfiguration {
    public static final Codec<SpiralTreeConfiguration> CODEC = RecordCodecBuilder.create((fields) -> {
        return fields.group(Registry.BLOCK.byNameCodec().listOf().fieldOf("validBlocks").xmap(ImmutableSet::copyOf, ImmutableList::copyOf).forGetter((v) -> {
            ...
            return v.blobHeight;
        }), IntProvider.codec(1, 3).fieldOf("capSize").forGetter((v) -> {
            return v.blobHeight;
        })).apply(fields, SpiralTreeConfiguration::new);
    });
    ...
    private final IntProvider capSize;

    public SpiralTreeConfiguration(Set<Block> validBlocks, BlockState stemMaterial, BlockState leafMaterial, IntProvider blobMass, IntProvider blobWidth, IntProvider blobHeight, IntProvider capSize) {
    ...
    public IntProvider getCapSize() {return this.capSize;}
