...
@EventBusSubscriber(modid = Astrological.MODID)
public class CommonEvents {
    ...
    public static void playerTick(PlayerTickEvent.Pre event) {
        ...
        if (!level.isClientSide() && !player.isSpectator() && !player.touchingUnloadedChunk()) {
            ...
            if (standingOn.is(AstrologicalBlocks.OCHRE_SELENITE.get())) {
                player.addEffect(new MobEffectInstance(MobEffects.SPEED, 80, 31));
            } else if (standingOn.is(AstrologicalBlocks.VERDANT_SELENITE.get())) {
                player.addEffect(new MobEffectInstance(MobEffects.JUMP_BOOST, 5, 9));
            ...
            for (int i = 0; i < neighboring.size(); i++) {
                ...
                if ((blockState.is(AstrologicalBlocks.TENDRILS.get()) || blockState.is(AstrologicalBlocks.TUMOR.get())) && !player.hasEffect(MobEffects.NAUSEA)) {
                    player.addEffect(new MobEffectInstance(MobEffects.NAUSEA, 120, 1));
    ...
    private static void teleportPlayer(Level level, Player player) {
        ...
        for(int i = 0; i < 16; ++i) {
            ...
            double d4 = Mth.clamp(player.getY() + (double)(player.getRandom().nextInt(16) - 8), (double)level.getMinY(), (double)(level.getMaxY() + ((ServerLevel)level).getLogicalHeight() - 1));
