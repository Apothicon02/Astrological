...
public class LargeCalcitePillarFeature extends Feature<LargeDripstoneConfiguration> {
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargeCalcitePillarFeature.LargePillar largepillarfeature$largepillar = makeCalcite(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargeCalcitePillarFeature.LargePillar largepillarfeature$largepillar1 = makeCalcite(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
                    LargeCalcitePillarFeature.WindOffsetter largepillarfeature$windoffsetter;
                    if (largepillarfeature$largepillar.isSuitableForWind(config) && largepillarfeature$largepillar1.isSuitableForWind(config)) {
                        largepillarfeature$windoffsetter = new LargeCalcitePillarFeature.WindOffsetter(blockpos.getY(), random, config.windSpeed);
                    } else {
                        largepillarfeature$windoffsetter = LargeCalcitePillarFeature.WindOffsetter.noWind();
                    }
    ...
    private static LargeCalcitePillarFeature.LargePillar makeCalcite(BlockPos pRoot, boolean pPointingUp, RandomSource pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
        return new LargeCalcitePillarFeature.LargePillar(pRoot, pPointingUp, pRadius, (double)pBluntnessBase.sample(pRandom), (double)pScaleBase.sample(pRandom));
    }
    ...
    static final class LargePillar {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel pLevel, LargeCalcitePillarFeature.WindOffsetter pWindOffsetter) {
            while(this.radius > 1) {
        ...
        void placeBlocks(WorldGenLevel pLevel, RandomSource pRandom, LargeCalcitePillarFeature.WindOffsetter pWindOffsetter) {
    ...
    static final class WindOffsetter {
        ...
        static LargeCalcitePillarFeature.WindOffsetter noWind() {
            return new LargeCalcitePillarFeature.WindOffsetter();
        }
