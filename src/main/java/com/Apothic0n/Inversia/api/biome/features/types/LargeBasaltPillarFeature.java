...
public class LargeBasaltPillarFeature extends Feature<LargeDripstoneConfiguration> {
    public LargeBasaltPillarFeature(Codec<LargeDripstoneConfiguration> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargeBasaltPillarFeature.LargePillar largepillarfeature$largepillar = makeBasalt(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargeBasaltPillarFeature.LargePillar largepillarfeature$largepillar1 = makeBasalt(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
                    LargeBasaltPillarFeature.WindOffsetter largepillarfeature$windoffsetter;
                    if (largepillarfeature$largepillar.isSuitableForWind(config) && largepillarfeature$largepillar1.isSuitableForWind(config)) {
                        largepillarfeature$windoffsetter = new LargeBasaltPillarFeature.WindOffsetter(blockpos.getY(), random, config.windSpeed);
                    } else {
                        largepillarfeature$windoffsetter = LargeBasaltPillarFeature.WindOffsetter.noWind();
                    }
    ...
    private static LargeBasaltPillarFeature.LargePillar makeBasalt(BlockPos pRoot, boolean pPointingUp, RandomSource pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
        return new LargeBasaltPillarFeature.LargePillar(pRoot, pPointingUp, pRadius, (double)pBluntnessBase.sample(pRandom), (double)pScaleBase.sample(pRandom));
    }
    ...
    static final class LargePillar {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel pLevel, LargeBasaltPillarFeature.WindOffsetter pWindOffsetter) {
            while(this.radius > 1) {
        ...
        void placeBlocks(WorldGenLevel pLevel, RandomSource pRandom, LargeBasaltPillarFeature.WindOffsetter pWindOffsetter) {
    ...
    static final class WindOffsetter {
        ...
        static LargeBasaltPillarFeature.WindOffsetter noWind() {
            return new LargeBasaltPillarFeature.WindOffsetter();
        }
