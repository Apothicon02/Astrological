...
public class InsomniaVentBlock extends Block {
    public InsomniaVentBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(BlockState blockState, Level level, BlockPos blockPos, RandomSource randomSource) {
        ...
        if (randomNumber < 2 && !level.getBlockState(blockPos.above()).isSolid()) {
            for(int j = 0; j < 1080; ++j) {
                ...
                level.addParticle(PowerParticleOption.create(ParticleTypes.DRAGON_BREATH, 1.f), d3, d8, d13, (Math.random() * (50) - 25)*0.01, (Math.random() * (500) - 250)*0.0142, (Math.random() * (50) - 25)*0.01);
            }
